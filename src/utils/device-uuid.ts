...
const navigator: Navigator = <any>window.navigator
...
export class DeviceUUID {
  ...
  public Agent: any
  constructor(private options: { resolution: any } = { resolution: {} }) {
    const defOptions = {
      version: false,
      language: false,
      platform: true,
  ...
  get(customData?: Recordable<unknown>) {
    let pref = 'a'
    const du: any = this.parse()
    const dua: any[] = []
